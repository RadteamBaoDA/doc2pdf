...
Implement logic convert microsoft office word to pdf with below requirements:
...
1. Scope: All Pages, Selection, or Specific Range.
2. Layout: Orientation (Portrait/Landscape), Pages per sheet, and Custom Margins.
3. Metadata: Include/Exclude Document Properties and Accessibility Tags.
4. Bookmarks: Generate from Headings or Word Bookmarks.
5. Compliance: PDF/A compliance toggle.
6. Optimization: Image quality optimization and font embedding (Bitmap text fallback).
7. All config can setting by config.yml and can apply config in each subfolder name in input folder.
8. Make sure can convert read-only file.
...
Step5: Implement config to config.yml and apply multi config base on folder name